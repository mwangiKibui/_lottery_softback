    ...
    for (let index = 0; index < numbers.length; index++) {
      ...
      if (!matchedNumbers.has(index)) {
        ...
        if (limitGameCategory != "BLT") {
          ...
          // Calculating the percentage limit.
        ...
        console.log(">>>> sub admin info >>>>>",subAdminInfo);

        if(subAdminInfo && subAdminInfo.subAdminId && subAdminInfo.subAdminId._id){
          // this is for sub admin  only.
          subAdminLimit = await Limits.aggregate([
            {
              $match: {
                subAdmin: subAdminInfo.subAdminId._id,
                lotteryCategoryName: lotteryCategoryName,
        ...
        if (hasSuperVisorId) {
          ...
          console.log(">>>> seller limit >>>>", sellerLimit)
          ...
          if (sellerLimit?.length > 0) {
            ...
            console.log("the sold quantity seller is ",soldQuantitySeller);
            ...
            console.log(">>> item number >>> " + item.number+ " the remaining quantity is " + remainingQuantitySeller);
          }
          ...
          console.log(">>> item number >>> " + item.number + " item amount "+item.amount+" the actual max amount price buy is " + actualmaxAmountPriceBuy)
        }

        // console.log(actualmaxAmountPriceBuy)

        // console.log("the item number is "+item.number);
        ...
        if (subAdminLimitsCalcId) {
          ...
          if (!updatedLimit) {
            ...
            const upsertedLimit = await LimitCalc.findOneAndUpdate(
              { _id: subAdminLimitsCalcId },
              { $set:{date: currentDate},$push: {soldState: newEntry } },
              { new: true }
            );
          }
        } else {
        ...
        if (otherLimitCalcId) {
          ...
          if (!updatedLimit) {
            ...
            const upsertedLimit = await LimitCalc.findOneAndUpdate(
              { _id: otherLimitCalcId },
              { $set:{date: currentDate},$push: { date: currentDate,soldState: newEntry } },
              { new: true }
            );
          }
        } else {
        ...
        // the quantity should be added here....
      }
    }
