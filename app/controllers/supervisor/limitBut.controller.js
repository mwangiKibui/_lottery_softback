...
exports.addLimitBut = async (req, res) => {
  ...
  const subAdmin = req.user.subAdminId;
  try {
    let newLimit = null;
...
exports.getLimitButAll = async (req, res) => {
  try {
    ...
    let subAdmin = req.user.subAdminId;

    let allQuery = {
      subAdmin,
      superVisor: req.user._id, // Ensure superVisor does not exist
