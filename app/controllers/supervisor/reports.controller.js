...
exports.getSaleReports = async (req, res) => {
  try {
    ...
    if (supervisorId == "" || supervisorId == undefined) {
      let sellers = await User.find({ superVisorName: req.user.userName }, { _id: 1 });
      sellerIds = sellers.map((item) => item._id);
      seller_query = { $in: sellerIds };
    } else {
      // get the sellers that belong to the supervisor.
      let supervisor = await User.findOne({ _id:supervisorId });
      let sellers = await User.find({ superVisorName: supervisor.userName}, { _id: 1 });
      sellerIds = sellers.map((item) => item._id);
      seller_query =  { $in: sellerIds };
    }
